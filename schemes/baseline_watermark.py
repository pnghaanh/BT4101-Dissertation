...
class BaselineWatermarkLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        gamma: float = 0.5,
        delta: float = 1.0,
        hash_context_width: int = 3,
    ):
        self.vocab_size = vocab_size
        self.gamma = gamma
        self.delta = delta
        self.hash_context_width = max(1, int(hash_context_width))
        self.rng = torch.Generator()
        self.secret_key = hashlib.sha256(b"baseline_watermark").digest()[:32]

    def _build_hash_prefix(self, token_ids: List[int]) -> List[int]:
        return token_ids[-self.hash_context_width :] if len(token_ids) >= self.hash_context_width else token_ids
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for batch_idx in range(batch_size):
            prefix = self._build_hash_prefix(input_ids[batch_idx].tolist())
...
class BaselineWatermarkDetector:
    ...
    def __init__(self, vocab_size: int = 50257, gamma: float = 0.5, hash_context_width: int = 3):
        self.vocab_size = vocab_size
        self.gamma = gamma
        self.hash_context_width = max(1, int(hash_context_width))
        self.rng = torch.Generator()
    ...
    def _build_hash_prefix(self, token_ids: List[int], token_pos: int) -> List[int]:
        start = max(0, token_pos - self.hash_context_width)
    ...
    def detect(self, token_ids: List[int], z_threshold: float = 1.96) -> Dict:
        ...
        for i in range(1, len(token_ids)):
            prefix = self._build_hash_prefix(token_ids, i)
            greenlist = self._get_greenlist(prefix)
