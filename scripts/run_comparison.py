...
def extract_signal_pattern(method: str, detector, token_ids, max_bits: int = 50):
    bits = []
    if method == "baseline":
        for i in range(1, len(token_ids)):
            prefix = detector._build_hash_prefix(token_ids, i)
            greenlist = detector._get_greenlist(prefix)
            bits.append(1 if token_ids[i] in greenlist else 0)
            if len(bits) >= max_bits:
                break
        return bits

    if method == "pcm":
        for i in range(detector.warmup, len(token_ids)):
            membership_idx = i - detector.warmup
            block_pos = membership_idx % detector.effective_block_size
            if block_pos == 0:
                continue

            block_start_idx = membership_idx - block_pos
            anchor_token_pos = detector.warmup + block_start_idx
            if anchor_token_pos >= i:
                continue

            greenlist_prefix = detector._build_greenlist_prefix(token_ids, i)
            greenlist = detector._get_greenlist(greenlist_prefix)
...
def main():
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        for method in methods:
            ...
            if method in ("baseline", "pcm"):
                wm_signal_before[method] = extract_signal_pattern(method, detectors[method], ids, max_bits=50)
        ...
        for attack_name, attack_cfg in attacks.items():
            ...
            for method in methods:
                ...
                if (
                    attack_name == "paraphrase"
                    and method in ("baseline", "pcm")
                    and len(signal_trace[method]) < 30
                ):
                    before_bits = wm_signal_before.get(method, [])
                    after_bits = extract_signal_pattern(method, detectors[method], attacked_wm_ids, max_bits=50)
                    overlap = min(len(before_bits), len(after_bits))
                    flip_count = sum(1 for i in range(overlap) if before_bits[i] != after_bits[i])
                    signal_trace[method].append(
                        {
                            "prompt_index": int(prompt_idx),
                            "before_bits": before_bits,
                            "after_bits": after_bits,
                            "overlap_len": int(overlap),
                            "flip_count": int(flip_count),
                            "flip_rate": float(flip_count / overlap) if overlap > 0 else 0.0,
                        }
                    )
    ...
    signal_path_root, signal_ext = os.path.splitext(args.output_json)
    signal_output_path = f"{signal_path_root}_signal_trace_paraphrase{signal_ext or '.json'}"
    with open(signal_output_path, "w", encoding="utf-8") as f:
        json.dump(signal_trace, f, indent=2)

    print(json.dumps(payload["summary"], indent=2))
    print(f"Saved results to {args.output_json}")
    print(f"Saved signal traces to {signal_output_path}")
